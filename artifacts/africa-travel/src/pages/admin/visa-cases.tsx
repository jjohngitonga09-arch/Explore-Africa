...
function VisaCaseDetailDialog({ id, open, onOpenChange }: { id: number | null, open: boolean, onOpenChange: (open: boolean) => void }) {
  const { data: visaCase, isLoading } = useGetVisaCaseAdmin(id as number, {
    query: {
      enabled: !!id && open,
      queryKey: getGetVisaCaseAdminQueryKey(id as number)
    }
  });
  ...
  const handleSave = () => {
    if (!id) return;
    updateStatus.mutate(
      { id, data: { status, adminNotes: notes || undefined } },
      {
        onSuccess: () => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-none sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <div className="space-y-6 pt-4">
            <div className="grid grid-cols-2 gap-4 bg-muted/20 p-4 border border-border">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">Payment: {visaCase.feePaid ? <span className="text-green-600 font-medium">Paid</span> : 'Pending'}</div>
              </div>
            </div>

            <div>
              <h3 className="font-medium uppercase text-xs tracking-wider text-muted-foreground border-b pb-2 mb-3">Submitted Documents ({visaCase.documents?.length || 0})</h3>
            ...
            <div className="bg-card border border-border p-5 space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Message to Applicant (Visible to client)</label>
                <Textarea 
    ...
    <div className="p-8">
      <div className="flex items-center mb-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Process client documents and manage embassy submissions.</p>
        </div>
      </div>
      ...
      <div className="bg-card border border-border shadow-sm">
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">Loading cases...</TableCell></TableRow>
            ) : cases?.map((visaCase) => (
