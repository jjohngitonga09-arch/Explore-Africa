...
console.log(`✓ ${insertedOrigins.length} origin + ${insertedDests.length} destination countries`);
...
const tours = [
  {
    ...
    included: ["4x4 safari vehicle with guide", "All game drives", "Full board tented camp", "Park entry fees", "Airport transfers"],
  ...
  },
  {
    ...
    description: "Explore Tanzania's two most celebrated ecosystems — from the endless Serengeti plains to the prehistoric Ngorongoro Crater teeming with flamingos and black rhino.",
    durationDays: 10,
  ...
  },
  {
    ...
    description: "From the drama of Table Mountain to whale-watching cliffs at Hermanus and the lagoons of Knysna — South Africa's Garden Route is 300 km of pure wonder.",
    durationDays: 12,
  ...
  },
  {
    ...
    description: "Journey through 5,000 years of civilisation — from the Great Pyramid at Giza to the colossal temples of Luxor and Karnak, and sail the Nile on a traditional dahabiya.",
    durationDays: 9,
    ...
    included: ["Expert Egyptologist guide", "Cairo + Luxor + Aswan 4-star hotels", "Nile cruise (2 nights)", "All entry fees", "Domestic flights Cairo–Luxor–Aswan"],
  ...
  },
  {
    ...
    durationDays: 8,
    destinationCountryId: dest("Zanzibar").id,
    basePrice: "1790.00",
...
// 4. Tour pricing per origin
const byTitle = (t: string) => insertedTours.find(x => x.title.startsWith(t))!;
const byCode = (c: string) => insertedOrigins.find(x => x.code === c)!;

const pricingSpec: Array<{ title: string; prices: Array<[string, string]> }> = [
  { title: "Masai Mara", prices: [["US","2490.00"],["GB","2290.00"],["AU","2690.00"],["DE","2390.00"],["CA","2550.00"]] },
  { title: "Serengeti", prices: [["US","3190.00"],["GB","2990.00"],["AU","3390.00"],["DE","3090.00"]] },
  { title: "Cape Town", prices: [["US","2890.00"],["GB","2690.00"],["AU","2990.00"]] },
  { title: "Morocco", prices: [["US","1890.00"],["GB","1790.00"],["FR","1690.00"],["DE","1750.00"]] },
  { title: "Nile", prices: [["US","2190.00"],["GB","2090.00"],["AE","1990.00"]] },
  { title: "Rwanda", prices: [["US","4490.00"],["GB","4290.00"],["AU","4690.00"]] },
  { title: "Zanzibar", prices: [["US","1790.00"],["GB","1690.00"],["AU","1890.00"],["AE","1650.00"]] },
  { title: "Uganda", prices: [["US","2790.00"],["GB","2590.00"],["AU","2890.00"]] },
];

const pricingRows: any[] = [];
for (const { title, prices } of pricingSpec) {
  const tour = byTitle(title);
  if (!tour) continue;
  for (const [code, price] of prices) {
    pricingRows.push({ tourId: tour.id, originCountryId: byCode(code).id, price });
  }
}
await db.insert(tourPricingTable).values(pricingRows);
console.log(`✓ ${pricingRows.length} tour pricing rows`);
...
const visaServices = [
  ...
  { name: "South Africa Visitor Visa", description: "Up to 90-day visitor visa for South Africa. Pre-checked documents so your application succeeds first time.", fee: "95.00", destinationCountryId: dest("South Africa").id, requirements: ["Valid passport (6+ months)", "Bank statements (3 months)", "Return ticket", "Travel itinerary", "Proof of accommodation"], isActive: true },
  ...
  { name: "Egypt Tourist eVisa", description: "Egyptian e-Visa processed entirely online. We submit, track, and deliver your approved visa electronically.", fee: "65.00", destinationCountryId: dest("Egypt").id, requirements: ["Valid passport (min 6 months)", "Credit card for government fee", "Travel itinerary", "Passport photo (digital)", "Return flight ticket"], isActive: true },
  { name: "Rwanda Tourist Visa", description: "30-day single-entry visa via Irembo. Includes gorilla trekking permit coordination for Rwanda tour holders.", fee: "80.00", destinationCountryId: dest("Rwanda").id, requirements: ["Valid passport (min 6 months)", "Bank statement", "Return ticket", "Passport photo", "Yellow fever certificate"], isActive: true },
  { name: "Uganda Tourist Visa", description: "Single-entry Uganda e-Visa valid 90 days. We expedite and alert you immediately on approval.", fee: "70.00", destinationCountryId: dest("Uganda").id, requirements: ["Valid passport (min 6 months)", "Bank statement (3 months)", "Return ticket", "Proof of accommodation", "Yellow fever certificate"], isActive: true },
  ...
  { name: "Zanzibar & Tanzania Entry Visa", description: "Combined Tanzania mainland + Zanzibar visa. Zanzibar is part of Tanzania — we handle the unified application.", fee: "85.00", destinationCountryId: dest("Zanzibar").id, requirements: ["Valid passport (min 6 months)", "Return ferry/flight ticket", "Bank statement", "Hotel booking", "Passport photo"], isActive: true },
];

const insertedVisa = await db.insert(visaServicesTable).values(visaServices as any).returning();
console.log(`✓ ${insertedVisa.length} visa services`);
